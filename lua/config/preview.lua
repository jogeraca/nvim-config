...
local function plantuml_preview()
  ...
  if diagram_name then
    ...
    output_file = vim.fn.expand('%:p:r') .. '.svg'
  end

  vim.fn.jobstart({ 'plantuml', '-tsvg', current_file }, {
    on_exit = function(_, code)
      vim.schedule(function()
        ...
        if vim.fn.filereadable(output_file) == 0 then
          print('[plantuml] SVG not found: ' .. output_file)
          return
        end

        -- Remove <?plantuml ...?> processing instruction from SVG
        vim.fn.system("sed -i 's/<[?]plantuml[^?]*[?]>//g' '" .. output_file .. "'")

        if not plantuml_viewers[output_file] then
          plantuml_viewers[output_file] = vim.fn.jobstart({ 'imv', '-b', 'ffffff', output_file }, {
            on_exit = function()
